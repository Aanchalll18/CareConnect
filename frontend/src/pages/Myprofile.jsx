...
//     }
//     else{
//       toast.error(data.message || 'failed to upload profile')
//     }
//     }
//     catch(e){
//       console.log(e)
...
const MyProfile = () => {
  ...
  const updatedUserProfileData = async () => {
    try {
      ...
      console.error(e);
      toast.error(e.response?.data?.message || "An error occurred while updating profile");
    }
  };
    ...
    userData && (
      <div className="max-w-lg mx-auto mt-10 p-8 bg-white rounded-3xl shadow-2xl transform hover:scale-105 transition duration-300 text-grey-500">
            ...
            <label htmlFor="image">
              <div className="inline-block relative cursor-pointer">
                <img
                  ...
                  alt="Profile Preview"
                />
                <img className="w-10 absolute bottom-1 ml-5" src={assets.upload_icon} alt="Upload Icon" />
              </div>
              <input
                onChange={(e) => setImage(e.target.files[0])}
                type="file"
        ...
        <div className="mt-6">
          <button
            className="w-full px-4 py-2 bg-purple2 text-white rounded-md shadow-md hover:bg-primary focus:outline-none focus:ring-2 focus:ring-purple2 transform hover:translate-y-1 transition-all"
            onClick={isEdit ? updatedUserProfileData : () => setIsEdit(true)}
          >
            {isEdit ? "Save Information" : "Edit"}
          </button>
        </div>
